...
<%
	...
	String POSID_I = UtilString.removeNull((String) request.getAttribute(ZSWM_MOVIMENTAZIONI.POSID));
%>
<%
		...
		<div class="panel panel-default">
			<div class="panel-body">
				...
				<form name="form" method="post" action="MainUrl" class="formCheckBatch">
					...
					<input type="hidden" name="<%=Constants.PagProvenienza %>" id="<%=Constants.PagProvenienza %>"  value="<%=Constants.URL.MOV_415 %>"/>
					<input type="hidden" class="l10 span text" id="<%=ZSWM_MOVIMENTAZIONI.BWART %>" name="<%=ZSWM_MOVIMENTAZIONI.BWART %>" value="415"/>
					<input type="hidden" class="l10 span text" id="<%=ZSWM_MOVIMENTAZIONI.SOBKZ %>" name="<%=ZSWM_MOVIMENTAZIONI.SOBKZ %>" value="Q"/>	
					
					<div class="row">

						<div class="page-header">
							<h4 class="text-center text-primary"><b>Movimentazione 415</b></h4>	
						</div>
						...
						<div class="form-group col-md-12 col-sm-12 col-xs-12">
							<label for="<%=ZSWM_MOVIMENTAZIONI.POSID %>">WBS di Destinazione</label>
	    					<input type="text" class="form-control input-lg cleanOnReset" id="<%=ZSWM_MOVIMENTAZIONI.POSID %>" name="<%=ZSWM_MOVIMENTAZIONI.POSID %>" value="<%=UtilString.removeNull(POSID_I)%>" maxlength="24" required/>
						</div>
					</div>
