		...
		<div class="col-md-6 col-md-offset-3 col-sm-12 col-xs-12">
			<div class="panel panel-default">
				<div class="panel-body <%=panelBodyPaddingReset%>">
						...
						<input type="hidden" name="<%=Constants.PagProvenienza%>" id="<%=Constants.PagProvenienza%>"  value="<%=Constants.URL.MOV_RIEPILOGO_541%>"/>					
						<input type="hidden" id="maxRow" name="maxRow" value="<%=lista.size()%>"/>
						<input type="hidden" name="action" id="action" value=""/>
						<input type="hidden" name="host" id="host" value="<%=Host.Url() %>"/>
						<input type="hidden" name="<%=ZFMWM_GET_MOV_NETWORK.EBELN %>" value="<%=I_EBELN %>"/>
						<input type="hidden" name="<%=ZFMWM_GET_MOV_NETWORK.EBELP %>" value="<%=I_EBELP %>"/>
						
						<input type="hidden" name="<%=ZFMWM_GET_MOV_NETWORK.BWART %>" value="<%=I_BWART %>"/>
	
					<div class="row">
						<div class="page-header <%=headerMarginBottomReset%>">
							<h4 class="text-center text-primary"><b>Lista Movimentazione 541</b></h4>
						</div>
					</div>
					...
					%>
						<div class="row">
							<p class="text-center text-muted"> Nessuna Movimentazione 541 trovata.</p>
						</div>
					<%
					}
					%>
							...
							<div class="row fullHeightContentRow">
								...
								<div class="row">
										<div class="col-md-6 col-sm-6 col-xs-6">
											<b class="text-muted">Posizione OdA: </b>
										</div>
										<div class="col-md-6 col-sm-6 col-xs-6">
											<p class="list-group-item-heading"><%=bean.getEBELP() != null ? bean.getEBELP() : "N/D" %></p>
										</div>
									</div>
									<div class="row">
										<div class="col-md-6 col-sm-6 col-xs-6">
