		...
		<div class="panel panel-default">
			<div class="panel-body <%=panelBodyPaddingReset%>">
				...
				<form id="form" name="form" method="post" action="MainUrl">          
					...
					<input type="hidden" name="<%=Constants.PagProvenienza %>" id="<%=Constants.PagProvenienza %>"  value="<%=Constants.URL.ELENCOUBICAZIONI %>"/>
					<input type="hidden" name="host" id="host" value="<%=Host.Url() %>"/>
				</form>

				<div class="row">
					<div class="page-header <%=headerMarginBottomReset%>">
						<h4 class="text-center text-primary"><b>Elenco Materiali</b></h4>
					</div>
				</div>
				...
				%>
					<div class="row">
						<p class="text-center text-muted"> Nessun OUbicazione trovata.</p>
					</div>
				<%
				}
				%>
			...
			<ul class="list-group">
				...
				<li class="list-group-item noUpdate goTo" data-hidden="action" data-href="pos_<%=i%>">
					...
					<p class="clearfix mb0">
						<span class="pull-right"><b class="text-muted">Tipo Magazzino: </b><%=tob.getLGTYP() != null ? tob.getLGTYP() : "N/D" %></span>
			...
			<div class="panel-body">
				<p class="text-center">
					<button type="button" class="btn btn-lg btn-outline-secondary btn-block goBack" data-hidden="action" data-href="<%=Constants.URL.RICERCA_UBICAZIONE%>">Indietro</button>
				</p>
			</div>
