...
import it.cube.demo.sap.mobile.rfc.OT.RfcCreaOT;


public class CreaOTAction implements ActionInterface, Constants{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		...
		tipoMov=pagProv.substring(0,pagProv.lastIndexOf("/"));
		tipoMov=tipoMov.substring(tipoMov.lastIndexOf("/")+1);
		urlMov= URL.MOVMAT+tipoMov+"/"+tipoMov+".jsp";
		urlRiepilogo= URL.MOVMAT+tipoMov+URL.RIEPILOGO+tipoMov+".jsp";
		urlDettaglio= URL.MOVMAT+tipoMov+URL.DETTAGLIO+tipoMov+".jsp";
...
//			}
			...
			pagDest=URL.MOVMAT+tipoMov+URL.RIEPILOGO+tipoMov+".jsp";
		}else {
			pagDest=URL.MENU;
		}
...
//****************************************************************************************************
	/** QUESTA è QUELLA CHE VIENE USATA
	 * crea un movimento
	 * @param request
	 * @param response
	 */
	public void creaMov(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			String wbsPos = request.getParameter("wbs");
			if (wbsPos == null || wbsPos.isEmpty())
			...
			if(wbs_list != null) {
				Integer pos = new Integer(wbsPos);
				wbs_list.get(pos).setZFLAG("X");
			}
			RfcCreaOT rfc= new RfcCreaOT(tipoMov);
			...
			rfc.setWbs_list(wbs_list);
			rfc.setListaMov(singoloMovimento);
			rfc.execute();
			esito=rfc.getEsito();
			msg=rfc.getMsg();
			pagDest=urlMov;
			wbs_list = rfc.getWbs_list();
			if(SapRfc.Error.equalsIgnoreCase(esito)){
				singoloMovimento.lastIndexOf(request);	
			}
			if ("I".equalsIgnoreCase(esito)) {
				request.getSession().setAttribute("wbs_list", wbs_list);
			} else 
				request.getSession().removeAttribute("wbs_list");
		}catch(Exception e){
			esito=SapRfc.Error;
		...
		if(SapRfc.Error.equalsIgnoreCase(esito) || "I".equalsIgnoreCase(esito)){
