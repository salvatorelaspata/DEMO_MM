...
public class Mov261Action implements ActionInterface, Constants{
...
//					RfcCheckBatch rfcBatch = UtilCheck.checkBatch(request, menge, matnr, lgort, werks, licha, charg, lgnum, lgtyp, lgpla, "", "true".equals(IS_GEST_PARTITA) ? "X" : "");
...
/**//**/
		if(URL.MENU.equals(action))
			pagDest=URL.MENU;
		else if(URL.MOV_RICERCA_261.equals(pagProv)) {
			if ((request.getParameter(ZFMWM_GET_MOV_NETWORK.AUFNR) == null || request.getParameter(ZFMWM_GET_MOV_NETWORK.AUFNR) == "") && action == "b3") {
				esito = Error;
				msg = "E' necessario inserire un Ordine di Produzione per poter procedere.";
				pagDest=URL.MOV_RICERCA_261;
			}
			else{
				ricerca(request, response);
			}
			
		} else if(URL.MOV_RIEPILOGO_261.equals(pagProv)){
			pagDest = URL.MOV_RICERCA_261;
			if (action!=null && action.contains("pos_")){
				dettaglio(request, response);
			}
		} else if(URL.MOV_261.equals(pagProv)){
			pagDest=URL.MOV_261;
			if("b1".equals(action)) {
				try {
					getLista(request);	
					pagDest=URL.MOV_RIEPILOGO_261;
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void reset(HttpServletRequest request, MovNetworkBean movDetail) {
		...
		movDetail.setLICHA(this.movNetworkInitial.getLICHA());
	...
	private void ricerca(HttpServletRequest request, HttpServletResponse response){
		...
		if("b1".equals(action))
			...
			String I_AUFNR = request.getParameter(ZFMWM_GET_MOV_NETWORK.AUFNR);
			request.setAttribute(ZFMWM_GET_MOV_NETWORK.AUFNR, I_AUFNR);
	...
	private void getLista(HttpServletRequest request) throws Exception{
		...
		try{
			...
			if(rfc.isEsitoOK()){
				...
				pagDest = rfc.getMovNetworkList().size() > 0 ? URL.MOV_RIEPILOGO_261 : URL.MOV_RICERCA_261;
			}
